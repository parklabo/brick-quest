...
export const deleteJob = onCall({ maxInstances: 10, region: 'asia-northeast1' }, async (request) => {
  ...
  if (!DELETABLE.has(data.status)) {
    throw new HttpsError('failed-precondition', `Job cannot be deleted (current: ${data.status})`);
  }
